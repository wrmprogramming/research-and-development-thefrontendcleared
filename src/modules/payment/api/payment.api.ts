...
class PaymentApi {
  ...
  // ========== متد جدید: دریافت مجموع مبلغ پرداختی یک قرارداد ==========
  async getTotalPaidByContract(contractId: number): Promise<number> {
    try {
      console.log(`🔍 Getting total paid for contract ${contractId}`);
      
      // دریافت تمام پرداخت‌های تایید شده این قرارداد
      const response = await axiosClient.get<PaginatedResponse<Payment>>(
        API_ENDPOINTS.PAYMENT.BASE,
        { 
          params: { 
            contract: contractId, 
            is_verified: true,
            page_size: 1000 // دریافت همه پرداخت‌ها
          } 
        }
      );
      ...
      console.log(`💰 Total paid for contract ${contractId}: ${totalPaid}`);
      console.log(`📊 Number of verified payments: ${payments.length}`);
      
      return totalPaid;
    } catch (error) {
      console.error('❌ Error getting total paid by contract:', error);
      return 0;
    }
  }
