// src/modules/research-committee/components/ResearchCommitteeStats.tsx
...
export const ResearchCommitteeStats: React.FC<ResearchCommitteeStatsProps> = ({ className = '' }) => {
  const { useStats } = useResearchCommittee();

  const [selectedYear, setSelectedYear] = useState<number | 'all'>('all');
  const [isFirstLoad, setIsFirstLoad] = useState(true);

  // ✅ دریافت استات با پارامتر سال - همیشه صدا زده میشه
  ...
  // ========== وضعیت بارگذاری (بعد از همه هوک‌ها) ==========
  if (isLoading) {
    return (
      <div className={`research-committee-stats ${className}`}>
    ...
    return (
      <div className={`research-committee-stats ${className}`}>
        <div className="stats-error">
          <AlertCircle size={24} />
          <p>خطا در دریافت آمار کمیته‌های تحقیقات</p>
        </div>
      </div>
    );
  }
...
export default ResearchCommitteeStats;
