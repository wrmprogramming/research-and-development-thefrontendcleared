// src/modules/research-committee/pages/ResearchCommitteeDetailsPage.tsx
...
import { ResearchCommitteeForm } from '../components/ResearchCommitteeForm';

interface ResearchCommitteeDetailsPageProps {
  onEdit?: () => void;
  onDelete?: () => void;
}
...
export const ResearchCommitteeDetailsPage: React.FC<ResearchCommitteeDetailsPageProps> = ({ onEdit, onDelete }) => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { useItem, delete: deleteCommittee, isDeleting } = useResearchCommittee();
...
<head>
  ...
  <title>کمیته تحقیقات - ${committee.session_number}</title>
  <style>
    ...
    .approvements-section h3 { font-size: 15px; font-weight: 700; color: #4f46e5; margin: 0 0 12px 0; padding-bottom: 8px; border-bottom: 2px solid #e9ecef; }
...
<div class="container">
  ...
  <div class="grid">
    <div>
      ...
      <div class="section"><h3>📝 دستور کار</h3>
        <div class="desc">${committee.order || '—'}</div>
      </div>
    </div>
    <div>
      <div class="section"><h3>📎 فایل‌ها</h3>
        ${committee.minutes_file ? `<div class="row"><span class="lbl">صورتجلسه</span><span class="val"><a href="${committee.minutes_file}" target="_blank" style="color:#4f46e5;">مشاهده فایل</a></span></div>` : ''}
        ${committee.attachment ? `<div class="row"><span class="lbl">فایل پیوست</span><span class="val"><a href="${committee.attachment}" target="_blank" style="color:#4f46e5;">مشاهده فایل</a></span></div>` : ''}
  ...
  <div class="footer">
    ...
    <span>📄 کد جلسه: ${committee.session_number}</span>
      ...
      saveAs(blob, `کمیته_تحقیقات_${committee.session_number}.html`);
  ...
  const exportJSON = () => {
    ...
    try {
      ...
      saveAs(blob, `کمیته_تحقیقات_${committee.session_number}.json`);
...
`;
      ...
      saveAs(blob, `کمیته_تحقیقات_${committee.session_number}.txt`);
    ...
    try {
      ...
      pdfContainer.innerHTML = `
        <div style="padding: 25px 30px; background: white; font-family: 'Vazir', 'B Nazanin', Tahoma, sans-serif; direction: rtl; max-width: 794px;">
          <div style="background: linear-gradient(135deg, #4f46e5, #7c3aed); color: white; padding: 25px 30px; border-radius: 12px; margin-bottom: 20px; text-align: center;">
            <div style="font-size: 22px; font-weight: 700;">گزارش کمیته تحقیقات</div>
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px;">
            <div>
              <div style="background: #f8fafc; border: 1px solid #e9ecef; border-radius: 8px; padding: 12px 14px; margin-bottom: 12px;">
                <div style="font-size: 12px; font-weight: 700; color: #4f46e5; margin-bottom: 6px; padding-bottom: 5px; border-bottom: 2px solid #e9ecef;">اطلاعات جلسه</div>
                <div style="display: flex; justify-content: space-between; padding: 3px 0; border-bottom: 1px solid #f3f4f6; font-size: 11px;">
                  <span style="color: #6b7280;">شماره جلسه</span>
                  <span style="font-weight: 600; color: #4f46e5;">${committee.session_number}</span>
                </div>
              ...
              <div style="background: #f8fafc; border: 1px solid #e9ecef; border-radius: 8px; padding: 12px 14px;">
                <div style="font-size: 12px; font-weight: 700; color: #4f46e5; margin-bottom: 6px; padding-bottom: 5px; border-bottom: 2px solid #e9ecef;">دستور کار</div>
                <div style="font-size: 11px; color: #374151; line-height: 1.7; text-align: justify;">${committee.order || '—'}</div>
              </div>
            </div>
            <div>
              <div style="background: #f8fafc; border: 1px solid #e9ecef; border-radius: 8px; padding: 12px 14px; margin-bottom: 12px;">
                ...
                ${committee.minutes_file ? `<div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid #f3f4f6;font-size:11px;"><span style="color:#6b7280;">صورتجلسه</span><span style="color:#4f46e5;">✓</span></div>` : ''}
                ${committee.attachment ? `<div style="display:flex;justify-content:space-between;padding:3px 0;font-size:11px;"><span style="color:#6b7280;">فایل پیوست</span><span style="color:#4f46e5;">✓</span></div>` : ''}
              ...
              ${approvementsHtml ? `
              <div style="background: #f8fafc; border: 1px solid #e9ecef; border-radius: 8px; padding: 12px 14px; margin-bottom: 12px;">
                <div style="font-size: 12px; font-weight: 700; color: #4f46e5; margin-bottom: 6px; padding-bottom: 5px; border-bottom: 2px solid #e9ecef;">مصوبات</div>
                ${approvementsHtml}
              </div>` : ''}
              <div style="background: #f8fafc; border: 1px solid #e9ecef; border-radius: 8px; padding: 12px 14px;">
      ...
      pdf.save(`کمیته_تحقیقات_${committee.session_number}.pdf`);
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'کمیته_تحقیقات');
      XLSX.writeFile(wb, `کمیته_تحقیقات_${committee.session_number}.xlsx`);
  ...
  if (isLoading) {
    return (
      <div className="research-committee-details-page">
        <div className="loading-container">
          <div className="spinner"></div>
          <p>در حال بارگذاری اطلاعات کمیته تحقیقات...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="research-committee-details-page">
        <div className="error-container">
          <AlertCircle size={64} />
          <h3>کمیته تحقیقات یافت نشد</h3>
          <p>کمیته مورد نظر با شناسه {id} در سیستم وجود ندارد</p>
          <button className="btn-back" onClick={() => navigate('/committees-research')}>
            <ArrowLeft size={16} />
  ...
  return (
    <div className="research-committee-details-page" ref={contentRef}>
      {/* Header */}
        ...
        <div className="header-left">
          ...
          <div className="header-title">
            ...
            <div>
              ...
              <span className="committee-subject-header">کمیته تحقیقات</span>
            </div>
          </div>
        </div>
  ...
  <div className="summary-card">
    ...
    <div className="summary-info">
      ...
      <span className="summary-value">{toPersianNumberFn(committee.approvements?.length || 0)}</span>
        ...
        <div className="info-grid">
          <div className="info-column">
            ...
            <div className="info-section">
              <h3><FileText size={18} /> دستور کار</h3>
              <p className="description-text">{committee.order || '—'}</p>
            </div>
          </div>
      ...
      <style>{`
  .research-committee-details-page {
    padding: 24px;
    min-height: 100vh;
    background: #f8fafc;
  }
  ...
  /* ✅ تغییر: approvement-item بنفش روشن */
  .approvement-item {
    background: #eef2ff;
    border: 1px solid #c7d2fe;
    border-radius: 6px;
...
export default ResearchCommitteeDetailsPage;