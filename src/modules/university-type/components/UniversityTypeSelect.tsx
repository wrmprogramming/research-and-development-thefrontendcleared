// src/modules/university-type/components/UniversityTypeSelect.tsx
...
export const UniversityTypeSelect: React.FC<UniversityTypeSelectProps> = ({
  value,
  onChange,
  ...
  const { useList } = useUniversityType();
  const { data: types = [], isLoading } = useList({});
  ...
  const handleSelect = (type: UniversityType) => {
    if (type.id === value) {
      onChange(null);
    } else {
      onChange(type.id);
  ...
  useEffect(() => {
    if (highlightedIndex >= 0) {
      const list = document.getElementById('university-type-select-list');
  ...
  return (
    <div 
      ref={containerRef}
      className={`university-type-select ${className} ${isOpen ? 'open' : ''} ${disabled ? 'disabled' : ''} ${error ? 'has-error' : ''}`}
    ...
    >
        ...
        <div className="select-value">
          ...
          {selectedType ? (
            <div className="selected-info">
              <span className="selected-name">{selectedType.name}</span>
              {selectedType.code && (
                <span className="selected-code">کد: {selectedType.code}</span>
              )}
            </div>
          ) : (
        ...
        <div className="select-dropdown">
          <div className="dropdown-search">
            ...
            <input
              ...
              placeholder="جستجوی نوع دانشگاه..."
              value={searchTerm}
          ...
          <div id="university-type-select-list" className="dropdown-list">
            {isLoading ? (
      ...
      <style>{`
        ...
        .university-type-select.open .select-control {
          border-color: #4f46e5;
          box-shadow: 0 0 0 3px rgba(79, 70, 229, 0.1);
        }

        .university-type-select.has-error .select-control {
          border-color: #dc2626;
        }

        .university-type-select.disabled .select-control {
          background: #f3f4f6;
          cursor: not-allowed;
          opacity: 0.7;
        }
