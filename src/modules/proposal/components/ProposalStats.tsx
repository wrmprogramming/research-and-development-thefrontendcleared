...
export const ProposalStats: React.FC<ProposalStatsProps> = ({ className = '' }) => {
  ...
  const winnerCount = isYearSelected
    ? yearStats?.winner_count || 0
    : stats?.winner_count || 0;
  ...
  }, [isYearSelected, stats, selectedYear]);
  ...
  }, [stats, selectedYear, isYearSelected]);
  ...
  const rfpDataToShow = showAllRfp
    ? filteredRfpData
    : filteredRfpData.slice(0, DISPLAY_LIMIT);
  const universityDataToShow = showAllUniversity
    ? filteredUniversityData
    : filteredUniversityData.slice(0, DISPLAY_LIMIT);
  ...
  const statItems = [
    {
      ...
      label: isYearSelected
        ? `کل پروپوزال‌های سال ${toPersianNumber(selectedYear)}`
        : 'کل پروپوزال‌ها',
    ...
    },
    {
      key: 'winner',
      label: isYearSelected
        ? `برنده‌های سال ${toPersianNumber(selectedYear)}`
        : 'برنده',
      value: toPersianNumber(winnerCount),
      icon: Trophy,
      color: '#059669',
      bgColor: '#d1fae5',
    },
    {
      key: 'not_winner',
      label: isYearSelected
        ? `غیربرنده‌های سال ${toPersianNumber(selectedYear)}`
        : 'غیربرنده',
      value: toPersianNumber(notWinnerCount),
      icon: XCircle,
      color: '#dc2626',
      bgColor: '#fee2e2',
    },
    {
      key: 'rfp_count',
      label: isYearSelected
        ? `RFPهای مرتبط با سال ${toPersianNumber(selectedYear)}`
        : 'تعداد RFPهای مرتبط',
      value: toPersianNumber(rfpCount),
      icon: TrendingUp,
      color: '#d97706',
      bgColor: '#fef3c7',
    },
  ...
  // ============================================================
  // returnهای شرطی بعد از همه هوک‌ها
  // ============================================================
  if (isLoading) {
    return (
  ...
  return (
    <div className={`proposal-stats ${className}`}>
          ...
          <div className="year-stats">
            {[...stats.by_year]
              ...
              .map((item) => {
                ...
                const winRate =
                  item.count > 0
                    ? Math.round((item.winner_count / item.count) * 100)
                    : 0;
                ...
                return (
                  ...
                  >
                    <span className="year-label">{toPersianNumber(item.year)}</span>
                    ...
                    <span className="year-stat-count">
                      {toPersianNumber(item.count)} پروپوزال
                    </span>
                    <span className="year-stat-winner">
                      <Trophy size={12} />
                      {toPersianNumber(item.winner_count)} برنده
                    </span>
                    <span className="year-stat-rate">
                      {toPersianNumber(winRate)}%
                    </span>
                  </div>
                );
              })}
          ...
          <h4 className="stats-detail-title">
            ...
            {isYearSelected && (
              <span className="year-badge">
                سال {toPersianNumber(selectedYear)}
              </span>
            )}
          </h4>
            ...
            {filteredRfpData.length > DISPLAY_LIMIT && (
              ...
              >
                {showAllRfp ? (
                  ...
                  <>
                    <ChevronDown size={16} />
                    مشاهده {toPersianNumber(filteredRfpData.length - DISPLAY_LIMIT)} مورد دیگر
                  </>
                )}
              </button>
            )}
        ...
        <div className="stats-detail">
          ...
          <div className="university-stats">
            {universityDataToShow.map((item) => {
              const maxCount = Math.max(
                ...filteredUniversityData.map((i) => i.count),
                1
              );
              const percentage = (item.count / maxCount) * 100;
              return (
                <div key={item.university_id} className="university-stat-item">
                  <span className="university-stat-name">
                  ...
                  <span className="university-stat-count">
                    {toPersianNumber(item.count)}
                  </span>
                </div>
              );
            ...
            {filteredUniversityData.length > DISPLAY_LIMIT && (
              ...
              >
                {showAllUniversity ? (
                  ...
                  <>
                    <ChevronDown size={16} />
                    مشاهده {toPersianNumber(filteredUniversityData.length - DISPLAY_LIMIT)} مورد دیگر
                  </>
                )}
              </button>
            )}
      ...
      <style>{`
        .proposal-stats { margin-bottom: 20px; }
        ...
        .year-badge { font-size: 12px; font-weight: 500; color: #4f46e5; background: #eef2ff; padding: 2px 10px; border-radius: 12px; margin-right: 8px; }
        ...
        .year-stats, .rfp-stats, .university-stats { display: flex; flex-direction: column; gap: 8px; }
        ...
        .year-stat-count { font-size: 13px; font-weight: 600; color: #1a1a2e; min-width: 80px; text-align: left; }
        .year-stat-item.selected .year-stat-count { color: #4f46e5; font-weight: 700; }
        .year-stat-winner { display: flex; align-items: center; gap: 4px; font-size: 12px; font-weight: 500; color: #059669; min-width: 70px; }
        .year-stat-rate { font-size: 12px; font-weight: 600; color: #6b7280; min-width: 40px; text-align: left; }
        .year-stat-item.selected .year-stat-rate { color: #4f46e5; }
        .rfp-stat-item, .university-stat-item { display: flex; align-items: center; gap: 12px; }
        .rfp-stat-name, .university-stat-name { font-size: 13px; font-weight: 500; color: #1a1a2e; min-width: 120px; word-break: break-word; }
        .rfp-stat-bar, .university-stat-bar { flex: 1; height: 6px; background: #f3f4f6; border-radius: 4px; overflow: hidden; min-width: 60px; }
        .rfp-stat-fill, .university-stat-fill { height: 100%; border-radius: 4px; transition: width 0.6s ease; }
        .rfp-stat-count, .university-stat-count { font-size: 13px; font-weight: 600; color: #1a1a2e; min-width: 30px; text-align: left; }
        .show-more-btn { display: inline-flex; align-items: center; gap: 6px; padding: 6px 14px; margin-top: 8px; border: 1.5px dashed #d1d5db; border-radius: 6px; background: transparent; color: #6b7280; font-size: 13px; cursor: pointer; transition: all 0.2s; width: fit-content; }
        .show-more-btn:hover { border-color: #4f46e5; color: #4f46e5; background: #eef2ff; }
        ...
        @media (max-width: 768px) {
          ...
          .year-stat-count { min-width: 60px; font-size: 12px; }
          .year-stat-winner { min-width: 50px; font-size: 11px; }
          .rfp-stat-name, .university-stat-name { min-width: 80px; font-size: 12px; }
