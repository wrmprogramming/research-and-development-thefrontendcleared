...
export const CommunicationStats: React.FC<CommunicationStatsProps> = ({ className = '' }) => {
  ...
  const yearStats = useMemo(() => {
    ...
    // ✅ محاسبه درست total_with_research:
    // تعداد مکاتباتی که research دارن، توی همون بازه‌ی ماه‌های فیلترشده.
    // چون بک‌اند نمی‌تونه per-month بده، فقط کل رو داریم.
    // پس از by_research استفاده می‌کنیم ولی محدود به هیچ فیلتر ماهی نمی‌شه.
    // راه‌حل: از total_with_research اصلی استفاده کن.
    let withResearch = 0;
    if (stats) {
      if (selectedYear === 'all') {
        withResearch = stats.total_with_research || 0;
      } else {
        // نسبت مکاتبات این سال به کل
  ...
  }, [filteredByMonth, stats, selectedYear]);
  ...
  return (
    <div className={`communication-stats ${className}`}>
        ...
        <div className="status-stats">
          {Object.keys(filteredByMonth).length === 0 ? (
            <div className="empty-state-small">
              <span>
                هیچ مکاتبه‌ای در{' '}
                {isYearSelected ? `سال ${selectedYear}` : 'سال‌های انتخاب شده'} ثبت نشده است
              </span>
            </div>
          ) : (
                ...
                return (
                  <div key={key} className="status-stat-item">
                    ...
                    <span className="status-stat-count">
                      {toPersianNumber(item.count)} مورد
                    </span>
                  </div>
                );
              })
        ...
        <div className="stats-detail">
          ...
          <div className="year-stats">
            {Object.entries(stats.by_year)
              ...
              .map(([year, data]) => {
                // ✅ همیشه object با { year, count }
                const count = data.count;
                const maxCount = Math.max(
                  ...Object.values(stats.by_year).map((v) => v.count)
                );
      ...
      <style>{`
        .communication-stats { margin-bottom: 20px; }
        ...
        .stats-detail { padding: 16px 20px; background: white; border-radius: 12px; border: 1px solid #e9ecef; margin-top: 12px; }
        ...
        .status-stat-item { display: flex; align-items: center; gap: 12px; }
        .status-stat-info { display: flex; align-items: center; gap: 8px; min-width: 100px; }
        .status-dot { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; }
        .status-stat-name { font-size: 13px; font-weight: 500; color: #1a1a2e; }
        .status-stat-bar { flex: 1; height: 6px; background: #f3f4f6; border-radius: 4px; overflow: hidden; min-width: 60px; }
        .status-stat-fill { height: 100%; border-radius: 4px; transition: width 0.6s ease; }
        .status-stat-count { font-size: 13px; font-weight: 600; color: #1a1a2e; min-width: 80px; text-align: left; }
        .year-stats { display: flex; flex-direction: column; gap: 8px; }
        .year-stat-item { display: flex; align-items: center; gap: 12px; cursor: pointer; }
        .year-stat-item:hover .year-stat-fill { opacity: 0.8; }
        ...
        .year-stat-bar { flex: 1; height: 6px; background: #f3f4f6; border-radius: 4px; overflow: hidden; }
        .year-stat-fill { height: 100%; border-radius: 4px; transition: width 0.6s ease; }
        .year-stat-count { font-size: 13px; font-weight: 600; color: #1a1a2e; min-width: 30px; text-align: left; }
        ...
        .stats-error { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 30px; background: #fee2e2; border-radius: 12px; color: #dc2626; gap: 8px; }
        .stats-error p { margin: 0; font-weight: 500; }
        .empty-state-small { text-align: center; padding: 16px; color: #9ca3af; font-size: 13px; }
        @media (max-width: 768px) {
          .stats-header { flex-direction: column; align-items: stretch; }
          .stats-grid { grid-template-columns: 1fr 1fr; }
          .status-stat-item { flex-wrap: wrap; }
