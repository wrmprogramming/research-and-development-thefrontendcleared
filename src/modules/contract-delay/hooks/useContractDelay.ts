// src/modules/contract-delay/hooks/useContractDelay.ts
...
export const useContractDelay = () => {
  ...
  const createMutation = useMutation({
    mutationFn: ({ data, onProgress }: { data: ContractDelayFormData; onProgress?: (p: number) => void }) =>
      contractDelayApi.create(data, onProgress),
    onMutate: () => setIsCreating(true),
    onSuccess: (_, variables) => {
      setIsCreating(false);
      queryClient.invalidateQueries({ queryKey: CONTRACT_DELAY_KEYS.all });
      queryClient.invalidateQueries({
        queryKey: CONTRACT_DELAY_KEYS.byContract(variables.data.contract_id),
      });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data, onProgress }: { id: number; data: Partial<ContractDelayFormData>; onProgress?: (p: number) => void }) =>
      contractDelayApi.update(id, data, onProgress),
  ...
  const create = useCallback(
    (data: ContractDelayFormData, onProgress?: (p: number) => void) =>
      createMutation.mutateAsync({ data, onProgress }),
    [createMutation]
  );

  const update = useCallback(
    (id: number, data: Partial<ContractDelayFormData>, onProgress?: (p: number) => void) =>
