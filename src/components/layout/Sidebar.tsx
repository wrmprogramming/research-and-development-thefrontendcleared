...
interface NavItem {
  ...
  requiredPermission?: string;  // ← codename مجوز
}

// ========== منوها با codename مجوزها ==========
const navigation: NavItem[] = [
  ...
  },
  {
    icon: Building2,
    name: 'مدیریت پایه',
    requiredPermission: 'base.province.view',  // ← اگه اینو داره، منو رو ببینه
    children: [
      ...
      { name: 'موضوع پروژه', href: '/project-subjects', requiredPermission: 'base.project_subject.view' },
      { name: 'محقق اصلی', href: '/person', requiredPermission: 'base.person.view' },
      { name: 'نوع پرداخت', href: '/payment-types', requiredPermission: 'financial.payment_type.view' },
    ],
  },
  {
    icon: BookOpen,
    name: 'مدیریت پژوهشی',
    requiredPermission: 'research.research.view',
    children: [
  ...
  },
  {
    ...
    requiredPermission: 'contract.contract.view',
    children: [
      { name: 'قرارداد', href: '/contract', requiredPermission: 'contract.contract.view' },
      { name: 'پیشرفت فیزیکی پروژه', href: '/progress', requiredPermission: 'contract.progress.view' },
      { name: 'تمدید قرارداد', href: '/contract-delays', requiredPermission: 'contract.delay.view' },
    ],
  },
  {
    icon: Building2,
    name: 'مدیریت مالی و پرداخت‌ها',
    requiredPermission: 'financial.payment.view',
    children: [
      { name: 'پرداخت‌های مالی', href: '/payment', requiredPermission: 'financial.payment.view' },
      { name: 'تسویه حساب', href: '/settlements', requiredPermission: 'financial.settlement.view' },
    ],
  },
  {
    icon: Users,
    name: 'کمیته‌ها',
    requiredPermission: 'committee.research.view',
    children: [
...
export function Sidebar({ isOpen }: SidebarProps) {
  ...
  const hasPermission = (permission?: string): boolean => {
    // اگه مجوزی تعریف نشده، نمایش بده
    if (!permission) return true;
    
    // اگه کاربر لاگین نکرده، نشون نده
    if (!user) return false;
    
    // سوپرادمین همه چیز رو می‌بینه
    if (user.is_superuser) return true;
    
    // اگه '*' داره، همه چیز رو می‌بینه
  ...
  const filteredNavigation = navigation
    .map((item) => {
      // اگه خود منو مجوز نداره، برو سراغ بچه‌ها
      if (item.children) {
        const filteredChildren = item.children.filter((child) =>
          hasPermission(child.requiredPermission)
        );
        
        // اگه هیچ بچه‌ای مجوز نداره، کل منو رو حذف کن
        if (filteredChildren.length === 0) return null;
        
        // اگه بچه‌ها مجوز دارن، منو رو با بچه‌های فیلترشده برگردون
        return { ...item, children: filteredChildren };
      }
      
      // اگه منو مجوز نداره، حذف کن
...
//   requiredPermission: 'can_manage_base',
//   children: [
//     { name: 'کاربران', href: '/users' },
//     { name: 'نقش‌ها', href: '/roles' },
//     { name: 'مجوزها', href: '/permissions' },
//   ],
// },
