...
class AxiosClient {
  ...
  private setupInterceptors(): void {
    ...
    this.client.interceptors.request.use(
      (config) => {
        ...
        // برای FormData، Content-Type رو حذف کن تا مرورگر خودش با boundary درست بذاره
    ...
    this.client.interceptors.response.use(
      (response: AxiosResponse) => {
        // ✅ پاسخ رو دست‌نخورده برگردون. unwrap توی متدهای get/post/... انجام می‌شه
        return response;
      },
      async (error: AxiosError) => {
        ...
        if (error.response?.status === 401 && !originalRequest._retry) {
          ...
          try {
            ...
            if (refreshToken) {
              // ⚠️ از axios خام استفاده کن تا از interceptor فعلی رد نشه (جلوگیری از loop)
  ...
  // ============================================================
  // Helper: unwrap کردن پاسخ
  // ============================================================
  /**
   * اگه پاسخ شکل `{ data: ... }` داشت، فقط `data` رو برگردون.
   * اگه `data` واقعاً `null` بود، هم `null` رو برگردون (نه کل body رو).
   * اگه اصلاً فیلد `data` نداشت، خود body رو برگردون.
   */
